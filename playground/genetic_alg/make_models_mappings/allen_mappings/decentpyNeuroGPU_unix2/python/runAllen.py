...
stim_file = h5py.File('../stims/allen_data_stims_10000.hdf5', 'r')
ntimestep = 10000
vs_fn = '../Data/VHotP'
...
def run_model(stim_ind,real_ind):
    ...
    print(volts_fn,'../Data/VHotP'+str(real_ind)+'.dat')
...
def stim_swap(idx, i):
    ...
    old_time = '../Data/times.csv'
    if os.path.exists(old_stim):
        os.remove(old_stim)
        os.remove(old_time)
    os.rename(r'../Data/Stim_raw' + str(i) + '.csv', r'../Data/Stim_raw' + str(idx) + '.csv')
    os.rename(r'../Data/times' + str(i) + '.csv', r'../Data/times.csv')
...
#     param_values =  np.array([6.07463069e-07,-6.66e+01, 8.21934290e+00, 3.78152611e+00, 4.63968241e-03,
#        7.13655768e-01, 1.61413034e+01, 5.64248751e-02, 5.90273482e-04,
#        1.02324873e-02, 1.73324892e-03, 4.64568954e+00, 3.74644220e-03,
#        7.45183842e-04]).reshape(1,-1)#np.array(orig_params).reshape(1,-1)
    #param_values = np.ones(np.array(orig_params).shape).reshape(1,-1)
    param_values = np.repeat(param_values, 20, axis=0) # 10 copies of orig params
