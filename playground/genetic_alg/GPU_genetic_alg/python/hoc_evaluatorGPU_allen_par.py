...
#         print(param_values.shape, "pvals shape!!!!!!!!")
        ...
        for i in range(0,nstims):
            ...
            if i == 0:
                ap_check_score = check_ap_at_zero(i, shaped_volts).reshape(-1,1)
                nospike_score = np.array([noSpikePen(volts, self.target_volts_list[i]) for volts in shaped_volts]).reshape(-1,1)
                spike_score = np.array([SpikePen(volts, self.target_volts_list[i]) for volts in shaped_volts]).reshape(-1,1)
            else:
                ap_check_score = np.append(ap_check_score, check_ap_at_zero(i, shaped_volts).reshape(-1,1),axis=1)
                nospike_score = np.append(nospike_score, np.array([noSpikePen(volts, self.target_volts_list[i]) for volts in shaped_volts]).reshape(-1,1), axis=1)
                spike_score = np.append(nospike_score, np.array([SpikePen(volts, self.target_volts_list[i]) for volts in shaped_volts]).reshape(-1,1), axis=1)
            ...
            if last_batch: #end of batch
                for ii in range(2):
                    ...
                    end_times.append(time.time())
                    shaped_volts = self.getVolts(idx)
                    ap_check_score[:,16+ii] = check_ap_at_zero(ii+16, shaped_volts)
                    nospike_score[:,16+ii] = np.zeros(nospike_score[:,16+ii].shape)#np.array([noSpikePen(volts, self.target_volts_list[16+ii]) for volts in shaped_volts])
                    spike_score[:,16+ii] = np.array([SpikePen(volts, self.target_volts_list[16+ii]) for volts in shaped_volts])
                    if idx == 0:
                        self.data_volts_list = shaped_volts #start stacking volts
                    else:
