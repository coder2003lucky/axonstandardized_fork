...
if 'orig_opt_stim_name_list' is objectives_file.keys():
    orig_opt_stim_name_list = objectives_file['orig_opt_stim_name_list'][:]
    orig_opt_stim_name_list = [e.decode('ascii') for e in orig_opt_stim_name_list]
...
#orig_params = [orig_params[i] for i in params_opt_ind]
...
def evaluate_score_function(stim_name_list, target_volts_list, data_volts_list, weights):
    ...
    for i in range(len(stim_name_list)):
        ...
        for j in range(len(score_function_ordered_list)):
            ...
            # todo: how can this be cleaner? need to use stim file somehowf from line 97
            map_idx = np.where(np.array(opt_stim_list) ==  str(stim_name_list[i]))[0][0]
            try:
                transformation = h5py.File(scores_path+orig_opt_stim_name_list[map_idx]+'_scores.hdf5', 'r')['transformation_const_'+curr_sf][:]
            except:
                transformation = None
            if curr_weight == 0:
                curr_score = 0
...
class hoc_evaluator(bpop.evaluators.Evaluator):
    def __init__(self):
        ...
        self.params = [bpop.parameters.Parameter(name, bounds=(abs(minval), abs(maxval))) for name, minval, maxval in params_]
    ...
    def evaluate_with_lists(self, param_values):
        print(np.array(param_values).shape, "PVAL SHAPE", param_values)
