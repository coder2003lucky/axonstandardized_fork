...
def main():
    ...
    for i in range(3,5):#range(nstims):
        orig_volts = run_model(param_values[0], [opt_stim_name_list[i]])[0]
        if run_model:
            np.savetxt("sweep_data/orig_volts",orig_volts, delimiter=",")
        else:
            orig_volts = np.genfromtxt("sweep_data/orig_volts", delimiter=",")
        ...
        for idx in range(num_params): # iterate through parameters, 15 params are hardcoded in for now... could be dynami
            if idx not in sweep_inds:
                print("skipping ", idx)
                total_count += len(color_rotation)
                continue
            ...
            flat_ax = axs.flatten()
            for ax in flat_ax:
                ax.plot(curr_target_volts, color="green", label="target", alpha=.5)
                ax.plot(orig_volts, color='black', label="Base", linewidth=.7) # unmodified parameters
            for color_idx, color in enumerate(color_rotation): # for each transformation we have a parameter set to plot
                ...
                if run_model:
                    np.savetxt("sweep_data/volts{}".format(total_count),curr_volts, delimiter=",")
                else:
                    curr_volts = np.genfromtxt("sweep_data/volts{}".format(total_count), delimiter=",")
                    
                # TODO: what is going wrong with first 10 volts?
                flat_ax[color_idx].plot(curr_volts, color=color, label=str(transformations[color_idx]), alpha=.8, linewidth=1)
                total_count += 1
                #plt.ylim(min(curr_target_volts), max(curr_target_volts)) 
                flat_ax[color_idx].legend()
            #plt.legend()
            fig.suptitle(labels[idx] + " @ " + str(param_values[0][idx] ))
            pdf.savefig(fig)
            plt.close(fig)
        pdf.close()
