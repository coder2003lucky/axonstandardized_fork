...
logging.info("using nCpus: " + str(multiprocessing.cpu_count()))
...
class hoc_evaluator(bpop.evaluators.Evaluator):
    ...
    def run_model(self,stim_ind, params):
        ...
        p_object = subprocess.Popen(['../bin/rank' + str(size) + '/neuroGPU'+str(global_rank),str(stim_ind), str(global_rank)])
        return p_object
    ...
    def wrap_eval(self,pair):
        ...
        pid = os.getpid()
        print(pid)
        cProfile.runctx("result[0] = self.eval_stim_sf_pair(pair)", globals(), locals(), 'profileLogs/prof%d.prof' % pid)
        return result[0]
    ...
    def evaluate_with_lists(self, param_values):
        ...
        self.num_gen = comm.bcast(self.num_gen, root=0)
        ...
        logging.info("MPI gen " + str(self.num_gen) + " took: " + str(eval_end - start_time_sim) \
        + " from rank" + str(global_rank))
