...
class hoc_evaluator(bpop.evaluators.Evaluator):
    def __init__(self):
        ...
        functions'),len(score_function_ordered_list)*nstims) #for 16 stims * 25 scorefunctions
        ########################################################################################
...
#       self.target_volts = nrnMreadH5() 
    ...
    def test_par(self,perm):
        ...
        for k in range(len(norm_scores)):
            if np.isnan(norm_scores[k]):
                norm_scores[k] = 1
        return np.reshape(norm_scores * curr_weight,(-1,1))
    ...
    def evaluate_with_lists(self, param_values):
        ...
        self.weights2 = np.repeat(self.weights,4)
        ...
        for i in range(0,nstims):
            ...
            if idx == nGpus-1:
                ...
                if i == nGpus-1:
                    self.targV = target_volts[:nGpus]
                    score = self.eval_in_par()
                else:
                    self.targV = target_volts[i:i+nGpus]
        ...
        print(np.array(score).shape, "SCORE SHAPE")
        return score
